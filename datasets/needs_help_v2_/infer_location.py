...
# Paths updated to the consolidated structure
INPUT_DIR = "/Users/hdon/Projects/Firebase/real-time/bsky-firehose/python/bsky/datasets/needs_help/07-08-2026"
OUTPUT_DIR = "/Users/hdon/Projects/Firebase/real-time/bsky-firehose/python/bsky/datasets/needs_help/07-08-2026/location_inferred/"
...
def parse_date_dir(dirname):
    """Convert MM-DD-YYYY directory name to datetime. Returns None for non-date directories."""
    try:
...
def check_for_pledges(text):
    """Returns True if the text contains 'pledge', '$', or 'dollar' (case-insensitive)."""
    text_lower = text.lower()
    return "pledge" in text_lower or "$" in text_lower or "dollar" in text_lower

def normalize_status(status):
    """Normalizes variation of statuses into standardized categories."""
    if not status:
...
def process_file(input_file, output_file):
    ...
    with open(input_file, "r", encoding="utf-8") as infile, \
        ...
        for line in infile:
            ...
            # Need text processing for location & pledge checking
            texts = collect_text(record)
            combined_text = "\n".join(texts)

            # 1. Evaluate Pledge Boolean Parameter
            record["has_pledge"] = check_for_pledges(combined_text)

            # 2. Location enrichment via Gemini API
            location = extract_location(combined_text)
            gemini_calls += 1

            record["extracted_location"] = location

            # Override/Correct location data fields dynamically
            ...
            if location["state_or_region"].strip():
                ...
                # Wipe old placeholder US states if the entry isn't in the USA
                record["state"] = ""

            if location["city"].strip():
                record["city"] = location["city"]

            # 3. Rule-based Status Normalization
            record["final_status"] = normalize_status(
                record.get("final_status", "")
            )

            # 4. NYCACC Fallback Guard
