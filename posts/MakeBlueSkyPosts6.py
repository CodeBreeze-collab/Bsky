...
class BlueskyClient:
    ...
    def __init__(self, handle, app_password):
        ...
        # Create scraper with enhanced stealth settings
        ...
        # Add high-reputation headers to bypass 403 blocks
    ...
    def validate_all_previews(self, posts):
        print("\n" + "=" * 50)
        print("🔍 PRE-FLIGHT EMBED VALIDATION REPORT")
        print("=" * 50)

        valid_list = []
        for i, post in enumerate(posts, 1):
            link = post["links"][0] if post["links"] else None
            print(f"Post {i}: {post['text'][:50]}...")

            if not link:
                print("  ⚠️ Result: No link provided (Text-only post).")
                valid_list.append(post)
                continue

            expanded = self.expand_url(link)
            embed = self._get_external_embed(expanded)

            if embed:
                print(f"  ✅ Result: Embed Ready! Title: '{embed['external']['title'][:40]}...'")
            else:
                print(f"  ⚠️ Embed failed — will fallback to Bluesky preview.")

            # Always consider the post valid; fallback will be used if embed fails
            valid_list.append(post)

        print("=" * 50)
        print(f"Summary: {len(valid_list)}/{len(posts)} posts are safe to post.")
        return valid_list
    ...
    def _get_external_embed(self, url: str):
        try:
            ...
            if "youtube.com/watch" in url or "youtu.be/" in url:
                ...
                thumb_blob = self._upload_blob(thumb_url)
                ...
                print(f"✅ YouTube embed built: {title}")
                return embed

            # --- Fallback for other URLs ---
            print(f"🔎 Fetching OG metadata from: {url}")
    ...
    def post_text(
        ...
        embed = None
        effective_policy = facet_policy

        if primary_text_link:
            expanded_url = self.expand_url(primary_text_link)

            if facet_policy == FacetPolicy.EMBED_ONLY:
                embed = self._get_external_embed(expanded_url)

                if not embed:
                    print("⚠️ Embed failed. Falling back to FACET_ONLY.")
                    effective_policy = FacetPolicy.FACET_ONLY

        # --- Step 2 & 4: Build facets using effective_policy and validate ---
        facet_links = links if effective_policy == FacetPolicy.FACET_ONLY else []
        facets = self._build_facets(text, facet_links, tags)

        errors = self.validate_post(text, facets, embed, effective_policy)
        if errors:
...
def run_posting_sequence(client: BlueskyClient, posts, facet_policy=FacetPolicy.EMBED_ONLY):
    # STEP 1: Run the report and filter posts
    safe_posts = client.validate_all_previews(posts)

    if not safe_posts:
        print("🛑 No valid posts to send. Exiting.")
        return

    # STEP 3: Safe Posting Loop
    print(f"🚀 Starting sequence...")
    for i, post_data in enumerate(safe_posts, 1):
        try:
            ...
            print(f"✅ Posted {i}/{len(safe_posts)}")
        except Exception as e:
            print(f"❌ Critical error on post {i}: {e}")
...
if __name__ == "__main__":
    FILE_PATH = "/Users/hdon/Projects/Firebase/real-time/bsky-firehose/python/bsky/posts/posts_v_search.jsonl"

    # --- Pre-flight check function (from your code) ---
    client, posts = pre_flight_check(FILE_PATH, Channel.V_SEARCH)
    ...
    # --- Posting sequence (from your code) ---
    if client and posts:
        run_posting_sequence(
            client,
            posts,
            facet_policy=FacetPolicy.EMBED_ONLY  # Safe: will auto-fallback to FACET_ONLY if embed fails
        )
    else:
